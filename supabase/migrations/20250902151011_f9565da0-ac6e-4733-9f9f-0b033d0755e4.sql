...
-- ⚡️ Índices (mantém os seus + adiciona únicos parciais e por vencimento)
-- (se já existir, não será recriado)
...
-- 🧾 Tabela para idempotência de webhooks (fila de eventos)
CREATE TABLE IF NOT EXISTS public.processed_events (
  ...
  last_error TEXT,
  event_created_at TIMESTAMPTZ, -- timestamp do Stripe (opcional)
  event_account TEXT            -- para connected accounts (opcional)
);

-- Índices para varrer a fila e relatórios por tempo
...
-- (Caso a tabela já existisse com DEFAULT TRUE, uniformiza)
